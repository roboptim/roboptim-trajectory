...
namespace roboptim
{
      ...
      nbp_ (p.size () / outputSize), uniform_ (true)
  {
    ...
    // number of control points should be at least order + 1 so that there is at least one segment in spline
    assert (nbp_ >= order_ + 1);
    // Fill vector of regularly spaced knots
    size_type m = nbp_ + order_ + 1;
    double delta_t = (tr.second - tr.first)/( m - order_ - 1 - order_);	//FIXME: check this
  ...
  BSpline<N>::BSpline (interval_t tr, size_type outputSize,
       ...
       nbp_ (p.size () / outputSize), knots_(knots), uniform_ (false)
   {
     ...
     assert (nbp_ >= order_ + 1);
     // calculated number of control points must match the recieved control point set
   ...
   */
  template <int N>
  typename BSpline<N>::cox_map BSpline<N>::cox_de_boor(size_type j, size_type n) const{
	  ...
	  if(n==0){		//end of recursion
		  ...
		  temp_params.setZero();
		  temp_params[0]=1.;
		  std::pair<cox_map_itr_t,bool> ptr = map.insert(std::make_pair(j,polynomial_t(0.,temp_params)));
		  if(debug)std::cout << label.str() << "return(end)    : " << ptr.first->second << std::endl;
		  return map;
	  }else{
		  // t_{j}
		  ...
		  if(isinf(p_1_rat.coefs_[1])){ // http://wolftype.com/ucsb/spatial/bspline.html, uniform b-splines
			  p_1_rat=monomial_t(t0);	//FIXME: this is probably not how it should work.
		  }
		  if(debug)std::cout << label.str() << "p_1_rat        : " << p_1_rat << std::endl;
		  cox_map p_1_cox = cox_de_boor(j,n-1);
		  cox_map p_1;
		  for(cox_map_itr_t itr=p_1_cox.begin();itr!=p_1_cox.end();itr++)
		  {
			  if(debug)std::cout << label.str() << "p_1_cox        : " << itr->first << " : " << itr->second << std::endl;
			  polynomial_t p_prod = itr->second * p_1_rat;
			  if(debug)std::cout << label.str() << "p_prod (1)     : " << p_prod << std::endl;
			  p_1.insert(std::make_pair(itr->first,p_prod));

		  }

		  polynomial_t p_2_rat = 1./(t1-tn1) * monomial_t(tn1);
		  if(isinf(p_2_rat.coefs_[1])){ // http://wolftype.com/ucsb/spatial/bspline.html, uniform b-splines
			  p_2_rat = Monomial<N>(tn1);	//FIXME: this is probably not how it should work.
		  }
		  if(debug)std::cout << label.str() << "p_2_rat        : " << p_2_rat << std::endl;
		  cox_map p_2_cox = cox_de_boor(j+1,n-1);
		  cox_map p_2;
		  for(cox_map_itr_t itr=p_2_cox.begin();itr!=p_2_cox.end();itr++)
		  {
			  if(debug)std::cout << label.str() << "p_2_cox        : " << itr->first << " : " << itr->second << std::endl;
			  polynomial_t p_prod = itr->second * p_2_rat;
			  if(debug)std::cout << label.str() << "p_prod (2)     : " << p_prod << std::endl;
		  ...
		  if(debug)std::cout << label.str() << "result of recursion branch" << std::endl;
		  for(cox_map_itr_t itr=p.begin();itr!=p.end();itr++)
		  {
			  if(debug)std::cout << label.str() << "B_" << j << "_" << itr->first << "  : " << itr->second << std::endl;
		  }
		  return p;
	  }
  }
  ...
  void BSpline<N>::computeBasisPolynomials ()
  {
    ...
    for (size_type j=0; j<nbp_; j++) {	//calculate basis polynomials for each interval of the knot vector
	  ...
	  {
		  if(debug)std::cout << "B_" << j << "_" << itr->first << "  : " << itr->second << std::endl;
...
#		endif
    ...
    if(order==0){
    	assert ( std::abs( polynomial_sum - 1. ) < std::numeric_limits<double>::epsilon()*1e6 );
    }
  }
