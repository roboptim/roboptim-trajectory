...
typedef spline_t::value_type value_type;
  ...
  boost::shared_ptr<spline_t> spline = boost::make_shared<spline_t>
    (std::make_pair (0., 1.), 1, params, "Cubic B-spline 1 (1D)", true);

  boost::shared_ptr<spline_t> spline2 = boost::make_shared<spline_t>
    (std::make_pair (0., 1.), 2, params2, "Cubic B-spline 2 (2D)", true);

  boost::shared_ptr<spline_t> spline3 = boost::make_shared<spline_t>
    (std::make_pair (0., 1.), 2, params3, "Cubic B-spline 3 (2D)", true);
  ...
  (*output)
    << (matplotlib
        ...
        << title ("Cubic B-spline (1D), with P4 = {2.5, 10}")
        << plot_spline (*spline2, step)
        << title ("Cubic B-spline (2D), with P4 = {2.5, 1}")
        << plot_spline (*spline3, step)
        << title ("Cubic B-spline (2D), with P4 = {2.5, 15}")
	);
